...
function movimientoManual(destino) {
  const posiciones = encontrarPosicionesDestino(destino)

  if (destino < contador) {
    for (
      let i = contador;
      i >= 0 && destino != arrayCoincidentePosicion[posiciones[0]];
      i--
    ) {
      empezarAnimacion(contador)
      contador++
    ...
    for (
      let i = contador;
      i <= animacionesNombre.length &&
      destino != arrayCoincidentePosicion[posiciones[1]];
      i++
    ) {
      empezarAnimacion(contador)
      contador--
